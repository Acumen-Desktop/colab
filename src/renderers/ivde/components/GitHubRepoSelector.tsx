...
export const GitHubRepoSelector = (props: GitHubRepoSelectorProps): JSXElement => {
  ...
  const [branches, setBranches] = createSignal<Array<{ name: string; commit: { sha: string }; protected: boolean }>>([]);
    ...
    try {
      ...
      if (selectedOrg() === "public") {
        ...
        const result = await githubService.searchRepositories(query, {
          sort: sortBy() === 'name' ? 'updated' : (sortBy() as 'updated' | 'stars'), // GitHub search API doesn't support name sorting
          per_page: 50,
        ...
        repos = await githubService.fetchOrganizationRepositories(selectedOrg()!, {
          sort: sortBy() === 'stars' || sortBy() === 'name' ? 'updated' : (sortBy() as 'created' | 'updated' | 'pushed'),
          per_page: 50
        });
    ...
    return (
      <div style="padding: 20px; text-align: center; color: #999;">
        ...
        <div style="font-size: 11px;">
          Connect your GitHub account in workspace settings to browse repositories.
        </div>
      </div>
    );
  }
              ...
              <div>
                ...
                <div
                  ...
                  style={{
                    ...
                    cursor: "pointer",
                    background: (props.selectedRepo?.id === repo.id && props.selectedBranch) ? "#1a4a6b" : "transparent"
                  }}
                  onMouseEnter={(e) => e.currentTarget.style.background = (props.selectedRepo?.id === repo.id && props.selectedBranch) ? "#1a4a6b" : "#333"}
                  onMouseLeave={(e) => e.currentTarget.style.background = (props.selectedRepo?.id === repo.id && props.selectedBranch) ? "#1a4a6b" : "transparent"}
                >
