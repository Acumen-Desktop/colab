...
const getInitialState = () => {
  ...
  if (!electrobun.rpc) {
    console.log("renderer getInitialState - RPC not ready, retrying in 100ms...");
        ...
        >
          ...
          {githubAuthUrl() && (
            <electrobun-webview
              ...
              style={{
                background: "#fff",
                width: "calc(100vw - 514px)", // Take remaining width after settings pane
                position: "absolute",
                top: "0px",
...
const GlobalSettings = () => {
  const [analyticsEnabled, setAnalyticsEnabled] = createSignal(state.appSettings.analyticsEnabled || false);
    ...
    <SettingsPaneFormSection label="Privacy & Analytics">
      <div style="margin-bottom: 16px;">
        <p style="color: rgb(204, 204, 204); font-size: 14px; line-height: 1.4; margin: 0 0 16px 0;">
          Help improve Colab by sharing anonymous usage data. This data helps us understand how features are used and identify issues.
        </p>
        
        <div style="background: rgb(45, 45, 45); border: 1px solid rgb(70, 70, 70); border-radius: 4px; padding: 16px; margin-bottom: 16px;">
          <h4 style="margin: 0 0 8px 0; color: rgb(235, 235, 235); font-size: 14px;">What data is collected?</h4>
          ...
          <h4 style="margin: 16px 0 8px 0; color: rgb(235, 235, 235); font-size: 14px;">What is NOT collected?</h4>
          <ul style="margin: 0; padding-left: 16px; color: rgb(204, 204, 204); font-size: 13px;">
      ...
      <div style="margin-top: 16px; padding-top: 16px; border-top: 1px solid rgb(70, 70, 70);">
        <div style="color: rgb(170, 170, 170); font-size: 12px;">
          <div style="margin-bottom: 4px;">
            <strong>Current Status:</strong> {analyticsStatus().enabled ? "Enabled" : "Disabled"}
          </div>
          <div style="margin-bottom: 4px;">
            <strong>Data Level:</strong> {analyticsStatus().level || "Community"} ({analyticsStatus().isAnonymous ? "Anonymous" : "Account-linked"})
          </div>
          <div style="margin-bottom: 4px;">
            <strong>Token Available:</strong> {analyticsStatus().hasToken ? "Yes" : "No"}
          </div>
          {hasBeenPrompted() && (
...
const PaneDivider = ({
  ...
  const styles: () => JSX.CSSProperties = () => ({
    ...
    "box-shadow": "inset 0 2px 4px rgba(0,0,0,0.4), inset 0 -2px 4px rgba(0,0,0,0.4)" 
  });
  ...
  createEffect(() => {
    ...
    if (_tab?.type === "terminal") {
      ...
      const folderName = currentPath ? 
        (currentPath === '/' ? 'root' : currentPath.split('/').pop() || 'root') : 
        'term';
    ...
    electrobun.rpc?.request
      ...
      .then((favicon: string) => {
        ...
        if (favicon &&
          ...
          console.log("No valid favicon found, keeping default icon:", defaultIcon);
          setIcon(defaultIcon);
        }
      })
      .catch((err) => {
  ...
  createEffect((prevHostname) => {
    ...
    if (_tab?.type === "web") {
      ...
      if (hostname !== prevHostname && prevHostname) {
        console.log("Favicon update - hostname changed from", prevHostname, "to", hostname);
        fetchFavicon(_tab);
      }
    ...
    <>
      <div
        ...
        style={{
          ...
          "box-shadow": isCurrentTab() ? "0px 0px 5px 0px #000" : "inset 0px 3px 6px -3px rgba(0, 0, 0, 0.4)",
...
const NodeSettings = () => {
  ...
  const previewNode = () => {
    ...
    if (settingsType === "add-node" || settingsType === "edit-node") {
      ...
      console.log("previewNode:", pNode);
      console.log("previewNode slate:", pNode?.slate);
      console.log("getSlateForNode result:", getSlateForNode(pNode));
      return pNode;
    }
    return null;
  };
  ...
  const [browserProfileNameInputValue, setBrowserProfileNameInputValue] = createSignal("");
  ...
  const shouldShowBrowserProfileNameHint = createMemo(() => {
    return browserProfileNameInputValue().trim() === "" && suggestedBrowserProfileName() !== "";
  });
  ...
  createEffect(() => {
    ...
    const selectedNodeType = 'selectedNodeType' in settingsData ? settingsData.selectedNodeType : undefined;
    ...
    if (!hasInitializedDropdown) {
      ...
      } else if (("slate" in _previewNode) && _previewNode.slate?.type === "project") {
        nodeType = "project";
      } else if (("slate" in _previewNode) && _previewNode.slate?.type === "web") {
        nodeType = "web";
      } else if (("slate" in _previewNode) && _previewNode.slate?.type === "agent") {
        nodeType = "agent";
      } else if (("slate" in _previewNode) && _previewNode.slate?.type === "repo") {
        nodeType = "repo";
      } else if (("slate" in _previewNode) && _previewNode.slate?.type === "devlink") {
        nodeType = "devlink";
      } else {
        ...
        const expandWhenReady = () => {
          ...
          if (node) {
            ...
            const openGitSlateWhenReady = () => {
              ...
              console.log(`Git folder poll attempt ${gitPollAttempts}, found:`, gitNode ? "yes" : "no");
    ...
    setState(
      produce((_state: AppState) => {
        ...
        if (__previewNode) {
          ...
          if (__previewNode.type === "dir" && "slate" in __previewNode && __previewNode.slate && "name" in __previewNode.slate) {
  ...
  const onBrowserProfileNameInputChange = () => {
    if (browserProfileNameRef) {
      ...
      const finalName = browserProfileNameRef.value.trim() || suggestedBrowserProfileName();
  ...
  const validateGitUrl = async (url: string) => {
    ...
    try {
      const result = await electrobun.rpc?.request.gitValidateUrl({ gitUrl: url });
      if (result?.valid) {
        setGitUrlValidation({ status: 'valid' });
      } else {
        setGitUrlValidation({ status: 'invalid', error: result?.error || 'Invalid repository URL' });
      }
    } catch (error) {
      setGitUrlValidation({ status: 'invalid', error: 'Failed to validate URL' });
    }
  };
  ...
  createEffect(async () => {
    ...
    setFolderInputLabel(
      state.settingsPane.type === "add-node" ? "Create folder" : "Rename folder"
    );
  });
                ...
                <div style="margin-top: 0px;background-color: transparent;border-left: 0px solid rgb(33, 33, 33);border-right: 0px solid rgb(33, 33, 33);border-radius: 0px;border-bottom: 1px solid rgb(33, 33, 33);">
                    ...
                    <Show when={!isProjectNode() && !isEditingNode()}>
                      <div class="field" style="margin-top: 0px;">
                        <div style="display: flex;flex-direction: column;">
                          ...
                          <div
                            style="background: #2b2b2b;border: 1px solid #212121;color: #efefef;padding: 8px;border-radius: 4px;font-size: 14px;font-weight: 600;"
                          >
                    ...
                    >
                        ...
                        <div style="position: relative;">
                          ...
                          <Show when={shouldShowProtocolHint()}>
                            <div style="font-size: 11px; color: #888; margin-top: 4px;">
                              Will save as: <span style="color: #aaa;">{finalUrl()}</span>
                            </div>
                          </Show>
                        </div>
                      </SettingsPaneField>
                      <SettingsPaneField label="Browser Profile Name">
                        <div>
                          ...
                          <Show when={shouldShowBrowserProfileNameHint()}>
                            <div style="font-size: 11px; color: #888; margin-top: 4px;">
                              Will save as: <span style="color: #aaa;">{finalBrowserProfileName()}</span>
                            </div>
                          </Show>
                        </div>
                      </SettingsPaneField>
                      ...
                      <Show when={state.settingsPane.type === "edit-node"}>
                        <SettingsPaneField label="">
                          <button
                            type="button"
                            onClick={onClickRemoveBrowserProfileSlateFromColabOnly}
                            style="cursor: pointer;background: #dd4444;color: white;font-weight: bold;border: none;padding: 10px;margin: 4px 0 8px;"
                          >
                            Remove Profile
                          </button>
                  ...
                  <Show when={getSlateForNode(previewNode())?.type === "repo"}>
                    ...
                    >
                      <SettingsPaneField label="Repository Source">
                        <div style="display: flex; gap: 8px; margin-bottom: 12px;">
                          <button
                            ...
                            style={{
                              background: useGitHubSelector() ? "#0969da" : "#2b2b2b",
                        ...
                        <SettingsPaneField label="Git Repository URL">
                          ...
                          <div style="position: relative;">
                            ...
                            <div style={{
                              ...
                              display: gitUrlValidation().status === 'idle' ? "none" : "block",
                              "font-size": "14px",
                            }}>
                      ...
                      <Show when={useGitHubSelector() && githubService.isConnected()}>
                        ...
                        <SettingsPaneField label="">
                          <div style="background: #2b2b2b; padding: 12px; border-radius: 4px; border: 1px solid #555;">
                            ...
                            <div style="font-size: 10px; color: #999; margin-bottom: 8px;">
                              Connect your GitHub account in workspace settings to browse your repositories.
                            </div>
                            <button
                              type="button"
                              onClick={() => {
